...
export function isIssuerAccount(account: Account | any): account is IssuerAccount {
    if (typeof account !== 'object') return false;
    if (typeof account.asset !== 'object') return false;
    return "featTypes" in account.asset;
}

export function isPersonAccount(account: Account | any): account is PersonAccount {
    if (typeof account !== 'object') return false;
    if (typeof account.asset !== 'object') return false;
